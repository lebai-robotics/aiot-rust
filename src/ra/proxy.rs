...
const DEFAULT_MSG_ID_HDSK: u64 = 0;
    ...
    async fn create_remote_proxy(&mut self) -> Result<()> {
        ...
        loop {
            tokio::select! {
                ...
                Some((session_id, data)) = self.local_rx.recv() => {
                    // 本地服务的数据的处理
                    // debug!("local_rx[{}]<-{:x?}", session_id, data);
                    write.send(protocol::gen_response(MsgType::ServiceProviderRawProtocol, 1, Some(session_id), None, &data)?.into()).await?;
                },
                            ...
                            match header.frame_type {
                                ...
                                MsgType::ServiceConsumerReleaseSession => {
                                    ...
                                    if let Some(session_id) = header.session_id {
                                        let id = self.session_list.release(session_id.clone());
                                        ...
                                        write.send(protocol::gen_response(MsgType::RespOk, header.frame_id, Some(session_id), None, payload.as_bytes())?.into()).await?;
                                    }
                                },
                                MsgType::ServiceProviderRawProtocol => {
                                    // 数据：云端传输给本地服务的数据
                                    match header.session_id {
                                        Some(session_id) => {
                                            if let Err(err) = self.session_list.write(session_id, payload).await {
                                                error!("write err: {}", err);
                                                write.send(protocol::gen_error(ErrorCode::SessionNonexistent, None, header.frame_id)?.into()).await?;
                                            }
                                        },
                                        None => {
                                            write.send(protocol::gen_error(ErrorCode::ParamInvalid, None, header.frame_id)?.into()).await?;
                                        }
                                    }
                                },
                                MsgType::RespOk => {
                                    ...
                                    info!("body {:?}", body);
                                    if header.frame_id == DEFAULT_MSG_ID_HDSK {
                ...
                _ = interval.tick() => {
                    ...
                    keepalive_cnt += 1;
                    // 发送通道保活信号
                    // let frame_id = rand_u64();
                    // write.send(protocol::gen_response(MsgType::KeepalivePing, frame_id, None, None, "")?.into()).await?;
                }
            }
        }
    }
    ...
    async fn new_session(&mut self, head: &MsgHead, body: &[u8]) -> Result<Vec<u8>> {
        ...
        match self.session_list.new(&body, self.local_tx.clone()).await {
            Ok(id) => {
                ...
                protocol::gen_response(
                    MsgType::RespOk,
                    head.frame_id,
                    Some(id),
                    head.service_type.clone(),
                    payload.as_bytes(),
                )
            }
